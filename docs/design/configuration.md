...
| `hostname` | string | optional | Falls back to the result of `os.Hostname()` when the TOML key is omitted, set to an empty string, or set to a whitespace-only string | Identifies the machine running bsky-cleaner in Slack notifications (see [0013_slack_notification_context](../tasks/0013_slack_notification_context/01_requirements.md)). The TOML key being absent and being set to `""` are not distinguished (same as `slack_allowed_host`); a whitespace-only value (e.g. `"  "`) is also treated as not set. In any of these cases, falls back to `os.Hostname()` |
