...
- **NF-005**: 実行結果（成功/失敗、削除件数、失敗した投稿の一覧等）は、標準出力への表示ロジックとは独立した構造化データ（interface）として内部的に保持すること。標準出力への変換はその構造化データからの一表現とし、[0006_slack_notification](../0006_slack_notification/01_requirements.md) が同じ構造化データを Slack 通知の入力として再利用できるようにする（本タスクでは Slack 通知自体は実装しない）。
