...
[0001_config](../0001_config/01_requirements.md)（設定管理）、[0002_atproto_client](../0002_atproto_client/01_requirements.md)（AT Protocol クライアント）、[0003_cleanup_engine](../0003_cleanup_engine/01_requirements.md)（クリーンアップエンジン）を組み立て、実際に動作する CLI バイナリ（`cmd/main.go`）として仕上げる。[プロジェクト概要](../../overview.md) の「安全性についての方針」にある通り、削除は不可逆な操作であるため、デフォルトは dry-run とし、明示的なフラグでのみ実削除を行う。
...
- **F-004**: 各コンポーネント（config/client/engine）の組み立てとエラー時の終了コード制御

### Out of Scope

- リトライ・実行タイムアウト（[0005_retry_timeout](../0005_retry_timeout/01_requirements.md) で扱う。この段階ではリトライなしの単純な組み立てを対象とする）
...
### F-004: コンポーネントの組み立てとエラー時の終了コード制御

**Acceptance Criteria**:
- **AC-10**: 設定読み込み・クライアント初期化（DID 解決を含む）・ログイン・一覧取得のいずれかが失敗した場合、削除処理に進まず非 0 の終了コードで終了する
- **AC-11**: 一部の投稿の削除が失敗した場合でも、残りの投稿の削除処理は継続され、最終的に失敗があったことが分かる形で非 0 の終了コードを返す

## 4. 非機能要件

- **NF-001**: `make fmt`・`make test`・`make lint` が成功する。
- **NF-002**: `cmd/main.go` はコンポーネントの組み立てに専念し、ビジネスロジック（削除対象判定等）を持ち込まない。
...
- リトライ・タイムアウトは、cron による定期実行・多重起動対策と関係が深く、CLI の基本的な組み立てとは独立して追加可能なため後続タスクとする。
...
- `bsky-cleaner --config <path>`（または `-c <path>`）で dry-run 一覧表示、`bsky-cleaner --config <path> --apply` で実削除ができる状態。
