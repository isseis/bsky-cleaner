...
- `cmd/main_test.go` は `package main` であり、`internal/atproto/did_test.go` の `stubSymbolicHostLookup`（非公開のパッケージ変数 `lookupIPAddr` を差し替える）にも `test_helpers.go` の `newTestClient`（`atproto.NewClient` を経由せず内部フィールドを直接組み立てる）にも一切アクセスできない。したがって `cmd/main_test.go` から `run`（`atproto.NewClient` を呼ぶ）を実行するテストは、`atproto.NewClient` が内部で行う DNS 解決（`internal/atproto/did.go` の `checkRequestHostSafety`/`validatePDSEndpoint` が呼ぶ `net.DefaultResolver.LookupIPAddr`）を実際に発生させてしまい、ネットワーク未接続の CI 環境では失敗する。この問題は、次の挙動を利用して回避する。`netip.ParseAddr` はホスト文字列を先に判定し、渡された文字列が IP リテラルであれば実際の名前解決を一切行わずに即座にそのアドレスを返す（Go 標準ライブラリ `net/lookup.go` の `lookupIPAddr` の実装。`internal/atproto/did_test.go` の `publicIPLiteral`/`stubSymbolicHostLookup` のコメントも同じ前提に依拠している）。具体的には、`cmd/main_test.go` のテストで用いる `BSKY_HANDLE`（`config.LoadCredentials()` 経由で `Credentials.Handle` に渡る値）を `did_test.go` と同じ RFC 5737 の公開 IP リテラル（例: `203.0.113.5`）にし、`MockHTTPDoer` に次の3レスポンスを固定で返させる。3つとも必須であり、特に3つ目を省略すると `validatePDSEndpoint` が「PDSサービスが見つからない」エラーで失敗し、意図と異なる理由でテストが失敗する（あるいは検出すべき異常を見逃して成立してしまう）。
  ...
  3. 上記 `serviceEndpoint`（`https://203.0.113.5`）に対する `validatePDSEndpoint` のホスト解決も同じ IP リテラルであるため、追加のレスポンスは不要（`validatePDSEndpoint` は HTTP リクエストを送らず DNS 解決のみを行う）。
