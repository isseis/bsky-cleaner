...
- [ ] `make lint` を実行し、`gosec` が `did.go` の DID ドキュメント取得（動的な URL を使う HTTP リクエスト構築）を指摘するか確認する（Phase 1 の同様の確認の続き、本書 2節 Phase 1 参照）。指摘された場合は、指摘箇所に限定した `//nolint:gosec // <理由>` を追加し、理由コメントに宛先ホストが `validatePDSEndpoint` で事前検証済みであることを明記する。
