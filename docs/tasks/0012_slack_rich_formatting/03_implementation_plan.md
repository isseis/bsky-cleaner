...
- [-] `make fmt && make test -run '^$' ./internal/notify/...`（コンパイルのみ確認、フェーズ3完了までテストは緑にならない前提）で構文エラーがないことを確認する。（実装はフェーズ1〜6を1コミットとして一括で行ったため、この中間時点のみを切り出した確認コマンドは個別実行していない。最終状態で `make fmt && make test && make lint` が緑であることは確認済み）
...
- [-] `make test ./internal/notify/...` を実行し、この時点で `buildPayload` 関連の既存テスト（フェーズ6で書き換える前の状態）がコンパイルエラーになることを確認したうえで、フェーズ6に進む（型変更に伴う一時的な赤は許容し、フェーズ6完了時点で緑に戻す）。（実装はフェーズ1〜6を1コミットとして一括で行ったため、この中間の赤状態を個別に確認するステップは実施していない。フェーズ6完了後の最終状態で `make test` が緑であることは確認済み）
...
- [x] PR を作成した（[#120](https://github.com/isseis/bsky-cleaner/pull/120)）
- [ ] PR がマージされた
