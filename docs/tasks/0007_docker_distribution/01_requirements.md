...
- **AC-04**: `schedule` の値が cron 式として構文的に妥当（5 フィールド、各フィールドはカンマ区切りの `*`／`*/n`／`a-b`／`a-b/n`／`a` の組み合わせで、値域が cron の仕様に準拠）であることを検証し、不正な場合は非 0 で終了する。`@daily` 等のマクロ形式は対象外（5 フィールド構成ではないためエラー終了する）
