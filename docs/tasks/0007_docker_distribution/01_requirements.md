...
**Acceptance Criteria**:
- **AC-04**: マルチステージビルドにより、ビルド用イメージと実行用の軽量イメージが分離されている
- **AC-05**: ベースイメージが、タグではなく digest（例: `alpine@sha256:...`）で参照されている
- **AC-06**: `docker build` によりイメージが正常にビルドできる

### F-003: エントリポイントスクリプトと内蔵 cron の連携

**Acceptance Criteria**:
- **AC-07**: コンテナ起動時、エントリポイントスクリプトが `print-schedule` を呼び出し、その結果を用いて crontab ファイルを動的生成する
- **AC-08**: 生成された crontab に従い、`bsky-cleaner --apply --config ...` が内蔵 cron（`supercronic` 等）経由で定期実行される
- **AC-09**: `print-schedule` がエラー終了した場合（不正な `schedule` 値等）、コンテナは起動を継続せず異常終了する（fail-closed）

### F-004: `docker-compose.yml` および `.env.example`

**Acceptance Criteria**:
- **AC-10**: `docker-compose.yml` に秘匿情報が直接書かれておらず、`environment:` で `.env` の変数を明示的に参照する形になっている
- **AC-11**: `.env.example` が値をダミー/空にした状態でコミットされており、初期セットアップの手引きとして機能する
...
- **NF-003**: 内蔵 cron ツール（`supercronic` 等）の選定理由がドキュメント化されている。
- **NF-004**: Go 1.26.2 以上でビルドできる CLI バイナリを前提とする。
