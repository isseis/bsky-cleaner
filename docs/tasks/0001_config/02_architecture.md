...
- **秘匿情報の分離**: TOML ファイルには秘匿情報を一切含めない（[プロジェクト概要](../../overview.md) の制約に従う）。app パスワードに加え、Slack Webhook URL（正常系・異常系）も秘匿情報として扱う。Webhook URL は値を知っていれば該当チャンネルに投稿できてしまうケーパビリティを持つ値であり、漏洩時の実害が app パスワードと同種であるためである。これら秘匿情報は環境変数からのみ取得し、専用の型（`SecretString`）でラップすることで、ログ・エラーメッセージへの意図しない漏洩を型システムのレベルで防ぐ。
...
flowchart TD
    ...
    TOML[("TOML 設定ファイル")]
    ENV[("環境変数<br>BSKY_HANDLE / BSKY_APP_PASSWORD /<br>Slack Webhook URL (正常系/異常系)")]
    ...
    alt ファイルが存在しない
        ...
        alt TOML 構文が不正
            ...
            alt 必須項目欠落 または 値が不正
                ...
                V-->>L: Config
                L->>E: os.LookupEnv() で BSKY_HANDLE・BSKY_APP_PASSWORD（必須）と<br>Slack Webhook URL 用の2変数（任意）を取得
                E-->>L: 値の有無
                L->>V: validateCredentials(values)
                alt 必須環境変数（Handle/AppPassword）が欠落、または Webhook URL の形式が不正
                    V-->>L: *FieldError
                    L-->>M: *FieldError
                else 検証成功
                    V-->>L: Credentials
...
// this at the point of use (e.g. building an authentication request or
// a Slack notification payload) and must never pass the result to a
// logger or formatter.
...
**`LoadCredentials` に `getenv` のようなコールバックを注入しない理由**: `func(key string) (string, bool)` のようなコールバックを引数で受け取る設計も検討したが、これは「関数の内部が任意の環境変数を読める」という、実際に必要な範囲（`BSKY_HANDLE`・`BSKY_APP_PASSWORD`・Slack Webhook URL 用の2変数の、計4つに限定）より広い権限を公開 API に持たせてしまう。テストは `t.Setenv()` で完結するため、この広い権限を許容してまで注入可能にする必要はない。`LoadCredentials()` は内部で `os.LookupEnv` をこの4つの変数名に対してのみ直接呼び出す。

### 3.3 コンポーネント責務表

| ファイル | 責務 | 関連 AC |
|---|---|---|
| `internal/config/config.go` | `Config` 型定義、`Load()`（TOML 読み込み + 検証） | AC-01, AC-02, AC-03, AC-04 |
| `internal/config/credentials.go` | `Credentials` 型定義、`LoadCredentials()`（環境変数読み込み + 検証） | AC-05, AC-06, AC-11, AC-13 |
...
- **`Value` は非秘匿フィールドのみに格納（AC-07, NF-003）**: `FieldError.Value` には、検証に失敗した実際の値の文字列表現を格納する。ただし格納するのは `Config`（TOML 由来）の3フィールド（`retention_days`・`schedule`・`execution_timeout_seconds`）に限る。`retention_days=0` のような設定ミスは運用上頻発する障害モードであり、実際にどんな値が入っていたかがエラーメッセージから分かることはオンコール対応上重要である。一方、`Credentials`（環境変数由来。`AppPassword` と Slack Webhook URL の双方を含む）の検証エラーでは `Value` を常に空文字列のままとする。Slack Webhook URL は値を知っていれば該当チャンネルへ投稿できてしまうケーパビリティを持つ値であり、`AppPassword` と同様に扱う（3.1 節参照）。エラーメッセージには「スキームが `https` ではない」のような検証内容のみを含め、URL 自体は含めない。`validateConfig()`（TOML 側、`Value` を埋める）と `validateCredentials()`（環境変数側、`Value` を埋めない）を別関数として分離しているため、実装時にこの使い分けを取り違えにくい。
...
- `internal/config/config_test.go`: 妥当な TOML・構文不正な TOML・存在しないファイル・必須項目欠落・未知のキー（タイプミスを想定）の各ケースを網羅する表駆動テスト（AC-01〜AC-04）。`retention_days`・`execution_timeout_seconds` の境界値テスト（`0`・負値・未設定・正の整数・`time.Duration` へのナノ秒変換でオーバーフローする極端に大きい値）も含む（AC-08〜AC-10）。
- `internal/config/credentials_test.go`: `t.Setenv()` で `BSKY_HANDLE`・`BSKY_APP_PASSWORD`（必須）を設定/未設定にした双方のケースを検証する（AC-05, AC-06）。Slack Webhook URL 用の2変数（任意）についても、未設定時はエラーにならないこと、設定時は `https` 以外のスキームで検証エラーになることを確認する（AC-11, AC-13）。
- `internal/config/secret_test.go`: `SecretString` を `fmt.Sprintf("%v", ...)`・`fmt.Sprintf("%#v", ...)`・`slog` の `Info` 呼び出しに渡し、出力に実際の値が含まれないことをアサートする（AC-07, NF-003）。`AppPassword`・Slack Webhook URL の双方の用途で同じ `SecretString` を使うため、テストは型レベルで一度書けば両方をカバーする。
