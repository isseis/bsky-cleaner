...
- これにより、ラップされた `*atproto.SSRFError`（リダイレクト拒否など）が無駄なリトライ予算を消費せず、速やかに呼び出し元へ伝播する。
...
`internal/retry` の `Doer` が、`Do` の結果として返されたエラーについて、そのチェーンのどこかに `permanentError` を満たすエラーがあり、かつその `Permanent()` が `true` を返す場合、当該エラーをリトライせずにそのまま呼び出し元へ返す。
...
- **NF-004**: 修正は永続判定の走査範囲の変更に限定し、無関係なリファクタリングを含めない（最小差分・YAGNI）。
