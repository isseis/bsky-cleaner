...
- **設定の一元化**: 実行スケジュールを含むすべての設定は TOML に一元化する。秘匿情報のみ `.env` に分離し、`docker-compose.yml` からは変数参照のみ行う
...
- 正常終了時と異常終了時で、それぞれ異なる Slack チャンネルに投稿できるようにする（同一チャンネルに統一する運用も可能とする）
...
Bluesky 公式 API を利用するため、入力データによる典型的な DoS（大量データによるバッファ溢れ等）は想定しにくいが、以下の攻撃ベクトルは考慮する必要がある。
...
- **削除処理の冪等性**: 投稿一覧取得後、削除実行までの間に対象投稿が既に削除されていた場合でも、クラッシュせず正常系として扱えるようにする
