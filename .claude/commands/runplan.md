...
- **Optional model tiering.** If this phase group's content has little interpretive freedom (e.g. wording/values are already fixed verbatim by the plan, a mechanical rename, a translation of already-approved prose), it may be delegated to a cheaper model via the Agent tool's `model` parameter, briefed with a self-contained prompt (exact file paths, exact required text/behavior, since the subagent starts without this conversation's context). Keep the phase group on the session's default model when it requires judgment about correctness — error handling, security-sensitive logic, measurement/timing boundaries, or summarizing an implementation's actual behavior in prose (the last of these still needs the default model even though it looks like "just docs": summarizing behavior requires understanding it, unlike restating already-fixed wording). Regardless of which model implements, step 7's discovery review always runs on the default model — its recall is not guaranteed complete even there (see `docs/tasks/0014_runplan_model_tiering/memo.md` for the data behind this guidance), so don't delegate discovery review itself for cost savings.
