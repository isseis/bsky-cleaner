...
- [ ] Any newly-authored static verification command (`rg`/`grep` pattern, script) in the plan is traced against representative sample content matching the actual naming/quoting/casing convention it targets — not merely checked for the presence of an expected-result clause. Flag a command that conflates two different conventions in one pattern (e.g. backtick-quoted lowercase struct tags vs. double-quoted uppercase env-var literals need separate extraction commands, not one combined regex).
- [ ] Every file named in the Test Strategy section (§4) as created in a given phase also appears in that phase's file list (§2) and has a corresponding checklist task creating it — cross-check file lists ↔ checklist bullets ↔ test-strategy file mentions, not just AC coverage.
